...
def _create_pots_from_draft(session_id: str, draft_id: str, count: int) -> list[dict]:
    ...
    for _ in range(max(1, count)):
        ...
        created.append({"pot_id": pot_id, "manage_url": manage_url})
    draft_ref.delete()
    db.collection("create_sessions").document(session_id).delete()
    db.collection("create_results").document(session_id).set({
        "pots": created,
...
# FastAPI
# =========================
app = FastAPI(title="PicklePot Backend — Owner Flows + Manual Paid + Finalize")
app.add_middleware(
    CORSMiddleware,
...
async def create_pot_session(payload: CreatePotPayload, request: Request):
    ...
    session = stripe.checkout.Session.create(
        mode="payment",
        line_items=[{
            "price_data": {"currency": "usd","product_data": {"name": f"Create Pot — {draft.get('name') or 'Tournament'}"},"unit_amount": amount_cents},
            "quantity": count,
        }],
    ...
    db.collection("create_sessions").document(session["id"]).set({"draft_id": draft_ref.id,"count": count,"createdAt": utcnow()})
...
async def create_checkout_session(payload: JoinPayload, request: Request):
    ...
    session = stripe.checkout.Session.create(
        mode="payment",
        line_items=[{"price_data": {"currency": "usd","product_data": {"name": f"Join Pot — {player_name}"},"unit_amount": amount_cents},"quantity": 1}],
...
def cancel_join(session_id: str, pot_id: Optional[str] = None, entry_id: Optional[str] = None, next: str = "/"):
    ...
    if pot_id and entry_id:
        ...
        es = entry_ref.get()
        if es.exists and not (es.to_dict() or {}).get("paid"):
            entry_ref.delete()
    return RedirectResponse(next, status_code=302)

# ---------- created-pots lookup ----------
@app.get("/created-pots")
def created_pots(session_id: str):
    pots = db.collection("pots").where("stripe_session_id", "==", session_id).stream()
    out = []
    for s in pots:
        pid = s.id
        link = db.collection("owner_links").document(pid).get()
        manage_url = (link.to_dict() or {}).get("manage_url") if link.exists else None
        if not manage_url:
            token = make_owner_token(pid)
            manage_url = f"{FRONTEND_BASE_URL}/manage?pot={pid}&key={token}"
            db.collection("owner_links").document(pid).set({"manage_url": manage_url}, merge=True)
        out.append({"pot_id": pid, "manage_url": manage_url})
    # fallback: if none found, try return stored create_results (if webhook already stored it)
    if not out:
        cr = db.collection("create_results").document(session_id).get()
        if cr.exists:
            out = (cr.to_dict() or {}).get("pots") or []
    return {"pots": out}

# ---------- NEW: finalize-create (fallback if webhook missed) ----------
@app.post("/finalize-create")
def finalize_create(session_id: str):
    # idempotent: if pots already exist, return them
    existing = [ {"pot_id": s.id, "manage_url": (db.collection('owner_links').document(s.id).get().to_dict() or {}).get('manage_url') }
                 for s in db.collection("pots").where("stripe_session_id", "==", session_id).stream() ]
    if existing:
        return {"pots": existing}
    # retrieve from stripe
    ses = stripe.checkout.Session.retrieve(session_id)
    if (ses.get("payment_status") or ses.get("status")) not in ("paid","complete"):
        raise HTTPException(400, "Session is not paid")
    md = ses.get("metadata") or {}
    if md.get("flow") != "create":
        raise HTTPException(400, "Not a create flow")
    draft_id = md.get("draft_id")
    count = int(md.get("count","1"))
    if not draft_id:
        raise HTTPException(400, "Missing draft_id in session metadata")
    created = _create_pots_from_draft(session_id, draft_id, count)
    return {"pots": created}

# ---------- Owner utilities (info, entries, manual paid) ----------
...
@app.get("/pots/{pot_id}/info")
def pot_info(pot_id: str, key: str):
    if not verify_owner_token(pot_id, key):
        raise HTTPException(401, "Invalid owner key")
    doc = db.collection("pots").document(pot_id).get()
    if not doc.exists:
        raise HTTPException(404, "Not found")
    data = doc.to_dict() or {}
    data.pop("owner_code_hash", None)
    data.pop("owner_token_salt", None)
    return {"pot_id": pot_id, "pot": data}

@app.get("/pots/{pot_id}/entries")
def list_entries(pot_id: str, key: str):
    if not verify_owner_token(pot_id, key):
        raise HTTPException(401, "Invalid owner key")
    q = db.collection("pots").document(pot_id).collection("entries").stream()
    items = [{"id": d.id, **(d.to_dict() or {})} for d in q]
    return {"entries": items}

@app.post("/pots/{pot_id}/entries/{entry_id}/mark-paid-manual")
def mark_paid_manual(pot_id: str, entry_id: str, body: ManualPaid, key: str):
    if not verify_owner_token(pot_id, key):
        raise HTTPException(401, "Invalid owner key")
    entry_ref = db.collection("pots").document(pot_id).collection("entries").document(entry_id)
    entry_ref.set({
        "paid": True,
        "paid_amount": int(body.amount_cents),
        "paid_at": utcnow(),
        "payment_method": f"manual:{body.method.lower()}",
        "note": body.note or None,
        "recorded_by": body.recorded_by or None,
    }, merge=True)
    return {"ok": True}
...
def unmark_paid(pot_id: str, entry_id: str, key: str):
    if not verify_owner_token(pot_id, key):
        raise HTTPException(401, "Invalid owner key")
    entry_ref = db.collection("pots").document(pot_id).collection("entries").document(entry_id)
    entry_ref.set({"paid": False,"paid_amount": None,"payment_method": None,"paid_at": None}, merge=True)
    return {"ok": True}
...
async def webhook(request: Request):
    ...
    if etype == "checkout.session.completed":
        ...
        if flow == "create":
            ...
            if draft_id:
                # idempotent check
                already = list(db.collection("pots").where("stripe_session_id","==",session["id"]).stream())
                if not already:
                    _create_pots_from_draft(session["id"], draft_id, count)
        elif flow == "join":
