...
async def create_checkout_session(payload: JoinPayload, request: Request):
    ...
    session = stripe.checkout.Session.create(
        mode="payment",
        line_items=[{
            "price_data":{"currency":"usd","product_data":{"name": f"Join Pot — {payload.player_name or 'Player'}"},"unit_amount": amount_cents},
            "quantity":1
        }],
        ...
        metadata={"flow":"join","pot_id": pot_id,"entry_id": entry_id,"player_email": payload.player_email or "", "player_name": payload.player_name or ""},
    )
...
# ----- Webhook: write manage links + owner code; cleanup drafts -----
@app.post("/webhook")
    ...
    if etype == "checkout.session.completed":
        ...
        if flow == "create":
            ...
            if pot_id and entry_id:
                ...
                ref.set({"paid": True, "paid_amount": session.get("amount_total"), "paid_at": utcnow(), "payment_method":"stripe","stripe_session_id": session["id"]}, merge=True)
